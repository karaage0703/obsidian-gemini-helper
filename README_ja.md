...
- **HTML インフォグラフィック プレビュー** - HTML コードブロックをプレビュー・保存（AI 生成のインフォグラフィックに最適）
...
- **変数** - `{selection}` で選択テキスト（未選択時はアクティブノートにフォールバック）、`{content}` でアクティブノートの内容を挿入
...
3. 以下を設定：
   ...
   - **Prompt template** - プロンプトテキスト（変数使用可）：
     - `{selection}` - 選択テキストに置換（未選択時はアクティブノートにフォールバック）
...
**デフォルトコマンド：**
- `/infographic` - コンテンツを HTML インフォグラフィックに変換（Preview/Save ボタン付き）
  - **注意**: デフォルトのプロンプトは英語です。日本語で出力したい場合は、設定からコマンドを編集し、プロンプトを日本語に書き換えてください（例：「以下の内容を HTML インフォグラフィックに変換して:\n\n{selection}」）
